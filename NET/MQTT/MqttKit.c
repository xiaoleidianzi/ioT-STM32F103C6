						...
						MQTT_PACKET_STRUCTURE *mqttPacket)
{
	...
	uint16 total_len = 15;//total_len开辟内容用 15=固定报头（消息类型（1）+长度（4））+可变报头（10）
	int16 len = 0, devid_len = strlen(devid);
	
	if(!devid)
		return 1;
	
	total_len += devid_len + 2;//设备名称长度devid_len+名称长度字节（2）
	...
	total_len += strlen(user) + strlen(password) + 4;//用户名称长user_len+名称长度字节（2） + 用户秘密password_len+密码长度字节（2）
	
	//分配内存-----------------------------------------------------------------------------
	MQTT_NewBuffer(mqttPacket, total_len);//内存长度包含（固定报头+可变报头+有效载荷）
	if(mqttPacket->_data == NULL)
		return 4;
...
/*************************************固定头部***********************************************/
	...
	//固定头部----------------------剩余长度值-----------------------------------------------
	len = MQTT_DumpLength(total_len - 5, mqttPacket->_data + mqttPacket->_len);//total_len - 5为数据协议规定真正的长度  mqttPacket->_len为数据写指针
	if(len < 0)
	{
...
uint8 MQTT_PacketSubscribe(uint16 pkt_id, enum MqttQosLevel qos, const int8 *topics[], uint8 topics_cnt, MQTT_PACKET_STRUCTURE *mqttPacket)
{
	...
	remain_len = 2 + 3 * topics_cnt + topic_len;//可变报头（报文标识符2）+ 有效载荷（主题长度定义字节2+主题内容长度topic_len   +服务质量要求1）
	
	//分配内存------------------------------------------------------------------------------
	MQTT_NewBuffer(mqttPacket, remain_len + 5);//5是固定报头最大字节数4+1
	if(mqttPacket->_data == NULL)
		return 3;
						...
						MQTT_PACKET_STRUCTURE *mqttPacket)
{
	...
	//总长度（不含固定报头）--------------------------------------------------------------------------------
	total_len = 2 + topic_len + payload_len ;  //2（描述主题长度自身字节） + topic_len（主题长度）+ payload_len (消息长度)
	...
	{
		MQTT_NewBuffer(mqttPacket, total_len + 5);//加上固定报头5个字节
...
//	说明：		
//==========================================================
uint1 MQTT_UnPacketPing(uint8 *rev_data)
{
